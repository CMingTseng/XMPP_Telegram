...
//    private static final Logger LOGGER = LoggerFactory.getLogger(TelegramBotService.class);
    ...
    private void init(TelegramBotConfig config) throws Exception {
        ...
        System.out.println("Verifying web hook..");
        if (url == null || url.isEmpty() || !url.equals(webHookUrl)) {
            System.out.println("Web Hook URL require changes updating to: {} .."+ webHookUrl);
            setWebhook(webHookUrl, "");
        } else {
            System.out.println("Web Hook is okay {}"+ webHookUrl);
        }
    }
    ...
    public BotApiMethod<?> onWebhookUpdateReceived(Update update) {
        if (update != null) {
            try {
                ...
                System.out.println("Failed to handle incoming update"+ e);
            }
        }
        return null;
    }
    ...
    private <T, R> void handleUpdate(Class<T> clazz, Update update, Function<Update, R> dataMapper, Function<R, User> idMapper, IThrowableFunction<T, Boolean> action) {
        ...
        for (T handler : handlers) {
            try {
                ...
                System.out.println("Exception caught on handler: {} error: {}");
            } catch (Exception e) {
...
//                        LOGGER.warn("Exception caught on handler: {}, message: {}", messageHandler.getClass().getSimpleName(), message, e);
    ...
    public void setWebhook(String url, String publicCertificatePath) throws TelegramApiRequestException {
        try {
            ...
            System.out.println("Web hook set: {}"+ jsonObject);
